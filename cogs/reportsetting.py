...
class reportsetting(commands.Cog):
    ...
    async def report_channel(self, ctx, on_off=None, channel: discord.TextChannel=None):
        if on_off is None:
            embed = discord.Embed(title="Ошибка", description="Укажите включение/выключение системы жалоб `s*канал-жалоб <on/off> <канал>`", color=discord.Color.red())
            ...
            if channel is None:
                embed = discord.Embed(title="Ошибка", description="Укажите канал для жалоб `s*report-channel <on/off> <channel>`", color=discord.Color.red())
                await ctx.send(embed=embed)
            else:
                if not collection.find_one({"guild_id": ctx.guild.id}):
